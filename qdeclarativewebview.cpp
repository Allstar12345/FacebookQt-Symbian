...
void QDeclarativeWebView::doLoadStarted()
{
    ...
    emit loadStarted();
    qDebug("WebViewCustom Loading Started");
}
...
void QDeclarativeWebView::pageUrlChanged()
{
    ...
    if ((d->url.isEmpty() && page()->mainFrame()->url() != QUrl(QLatin1String("qrc:/qml/StarBrowser/test.html")))
        || (d->url != page()->mainFrame()->url() && !page()->mainFrame()->url().isEmpty()))
    {
        d->url = page()->mainFrame()->url();
        if (d->url == QUrl(QLatin1String("qrc:/qml/StarBrowser/test.html")))
            d->url = QUrl();
        emit urlChanged();
    }
}
...
void QDeclarativeWebView::setPage(QWebPage* page)
{
    ...
    page->settings()->setAttribute(QWebSettings::SpatialNavigationEnabled, true);
    page->settings()->setAttribute(QWebSettings::FrameFlatteningEnabled, true);
    page->settings()->setLocalStoragePath("E:/AllstarSoftware/FacebookQt/LocalStorage/");
    page->settings()->setIconDatabasePath("E:/AllstarSoftware/FacebookQt/IconDatabase/");
    page->settings()->setOfflineStoragePath("E:/AllstarSoftware/FacebookQt/OfflineStorage/");
    page->settings()->setOfflineWebApplicationCachePath("E:/AllstarSoftware/FacebookQt/WebStorage/");
}
...
{
    emit imageRequested();
// TRAP_IGNORE(CAknDiscreetPopup::ShowGlobalPopupL((_L("Launching Default Browser")), (_L("Downloading Unsupported")),KAknsIIDNone, KNullDesC));
}
...
{
   /* QString filename;
      CDesCArrayFlat* fileArray = new CDesCArrayFlat(1);
      ...
      if(selectionOk)
      {
          filename = QString::fromUtf16(fileArray->MdcaPoint(0).Ptr(),
                                        fileArray->MdcaPoint(0).Length());
